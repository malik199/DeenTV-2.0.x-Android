    ...
    myApp.controller('DeenTv_Controller', ['$scope', '$http', function($scope, $http) {
        ...
        $scope.playvideo = function(currentItem) {			
            if (currentItem.custom_fields.dp_video_code) {
                ...
                $("#ytplayer").html('<iframe width="100%" height="100%" src="http://www.youtube.com/embed/' + currentItem.custom_fields._tern_wp_youtube_video + '?autoplay=1&wmode=opaque&rel=0&showinfo=0&modestbranding=0" frameborder="0" allowfullscreen></iframe>');
                gotoWatchPage()
            } else if (currentItem.custom_fields.dp_video_url) {
                var parsedID = getYoutubeID(currentItem.custom_fields.dp_video_url.toString());
                $("#ytplayer").html('<iframe width="100%" height="100%" src="http://www.youtube.com/embed/' + parsedID + '?autoplay=1&wmode=opaque&rel=0&showinfo=0&modestbranding=0" frameborder="0" allowfullscreen></iframe>');
                gotoWatchPage()
            } else {
